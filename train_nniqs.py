...
def train(train_loader, val_loader, model, optimizer):
    ...
    for batch in tqdm(val_loader, leave=False, desc='test'):
        for k, v in batch.items():
            batch[k] = v.cuda()
...
def main(config_, save_path):
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        train_loss, test_loss = train(train_loader, val_loader, model, optimizer)
        ...
        log_info.append('test: loss={:.4f}'.format(test_loss))
        ...
        if (epoch_val is not None) and (epoch % epoch_val == 0):
            ...
            log_info.append('test: psnr={:.4f}'.format(val_res))
        ...
        onedata = [epoch, train_loss, test_loss, val_res]
        data.append(onedata)
